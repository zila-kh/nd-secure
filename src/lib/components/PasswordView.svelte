<script lang="ts">
  ...
  async function loadMore() {
    ...
    loading = true;
    try {
      const page = await vaultApi.credentialPage(cursor, 100, appliedSearch, appliedProject, appliedEnvironment);
  ...
  async function refresh() { items = []; cursor = null; hasMore = true; await loadMore(); }
  async function applyFilters() {
    appliedSearch = search.trim();
    appliedProject = scopeFilter === 'central' ? '__central__' : scopeFilter === 'project' ? (projectFilter.trim() || '__project__') : (projectFilter.trim() || null);
    appliedEnvironment = environmentFilter.trim() || null;
    await refresh();
  }
  async function clearFilters() { search = ''; appliedSearch = ''; scopeFilter = 'all'; projectFilter = ''; environmentFilter = ''; appliedProject = null; appliedEnvironment = null; await refresh(); }
  function addNew() { editing = null; editorOpen = true; editorKey += 1; }
  async function openItem(item: CredentialSummary) { try { editing = await vaultApi.credentialDetail(item.id); editorOpen = true; editorKey += 1; } catch (cause) { error = String(cause); } }
  function closeEditor() { editorOpen = false; editing = null; }
  async function saved(_saved: CredentialDetail) { closeEditor(); await refresh(); }
  async function deleted(_id: string) { closeEditor(); await refresh(); }
  onMount(() => { void loadMore(); });
</script>

<section class="animate-fadeIn flex h-full min-h-0 flex-col gap-4">
  <header class="flex flex-wrap items-center justify-between gap-3"><div><h2 class="text-2xl font-semibold tracking-tight">Credential Manager</h2><p class="text-sm text-muted-foreground">Encrypted logins, secrets, history, custom fields, folders, project scopes, and TOTP.</p></div><div class="flex gap-2"><Button variant="secondary" size="sm" on:click={refresh} disabled={loading}><RefreshCw size={16} class={loading ? 'animate-spin' : ''} /> Refresh</Button><Button size="sm" on:click={addNew}><Plus size={16} /> New credential</Button></div></header>
