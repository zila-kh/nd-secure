...
fn derive_password_key(
    ...
    let params = Params::new(memory_kib, iterations, parallelism, Some(32)).map_err(|_| VaultError::Crypto)?;
...
mod tests {
    ...
    fn legacy_header_migrates_to_envelope_without_changing_domain_keys() {
        ...
        assert_eq!(state.domain_key(crate::crypto::GALLERY_DOMAIN).unwrap().as_ref(), expected_gallery.as_ref());
    ...
    fn master_password_change_preserves_encrypted_domain_keys() {
        ...
        state
            ...
            .unwrap();
        state.lock();
        assert!(state.unlock(Zeroizing::new(old_password.to_owned())).is_err());
        state.unlock(Zeroizing::new(new_password.to_owned())).unwrap();
        let after = state.domain_key(crate::crypto::CREDENTIALS_DOMAIN).unwrap();
